...
async def on_message(message): #when a message is sent in a channel Artie can see...
    ...
    if message.content.lower().startswith('~define'): #if the message starts with ~define
        ...
        definitionMessage = message.content.split(' = ') #split the message by the equals sign
    ...
    if configFile['BotAPI']['convowakeword'].lower() in message.content.lower(): #if the bot's name is in the message...
        for key in coreArtFile['art::core']: #iterate over the custom responses..
            if key in message.content.lower(): #if a response trigger is in the message..
                await type_message(message.channel, coreArtFile['art::core'][key]) #type and send the response using the type_response funtion
                return #stop on_message here
        for key in learnedArtFile['art::learned']: #iterate over the custom responses..
            if key in message.content.lower(): #if a response trigger is in the message..
                await type_message(message.channel, learnedArtFile['art::learned'][key]) #type and send the response using the type_response funtion
                return #stop on_message here
...
async def on_ready(): #when the bot is connected and logged into Discord..
    ...
    print('------')
    await client.change_presence(game=discord.Game(name="type ~info")) #Set the playing status to display moodFloat's value
